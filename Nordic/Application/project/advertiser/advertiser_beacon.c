...
static nrf_radio_request_t * m_configure_next_event(void)
{
...
static uint32_t m_request_earliest(enum NRF_RADIO_PRIORITY priority)
{
...
static uint8_t * m_get_adv_packet(void)
{
    ...
    adv_pdu[offset++] =  BLE_APPS_MANUFACTURER_SPECIFIC_DATA;
...
void beacon_adv_init(void)
{
    ...
    memcpy(beacon_init.uuid.uuid128, Bsp.uuid, 16);
    beacon_init.adv_interval  = BEACON_ADV_INTERVAL;
    beacon_init.major         = Bsp.major;
    beacon_init.minor         = Bsp.minor;
    beacon_init.manuf_id      = Bsp.manuf_id;
