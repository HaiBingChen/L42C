...
bma2x2_t * p_bma2x2;
//unsigned char V_BMA2x2RESOLUTION_U8R = 3; /* Based on Bit resolution value should be modified */
...
signed char write_bus_bma2xx(unsigned char device_addr, unsigned char register_addr, unsigned char * data, unsigned char length)
{
  return I2cMultipleByteWrite(device_addr<<1, register_addr, data, length);
  
}

signed char read_bus_bma2xx(unsigned char device_addr, unsigned char register_addr, unsigned char * data, unsigned char length)
{
  return I2cMultipleByteRead(device_addr<<1, register_addr, data, length);
}


signed char read_burst_bma2xx(unsigned char device_addr, unsigned char register_addr, unsigned char * data, unsigned int length)
{
  return I2cMultipleByteRead(device_addr<<1, register_addr, data, length);
}

void delay_msec_bma2xx( unsigned int length )
{
...
bma2x2_t gbma2x2;
#define DELAY_I2C  DelayMs(2)

//进入Standby mode
void Bma250E_Standby(void)
{
	uint8 data;
	data=0;
	...
	bma2x2_get_mode (&data);
	if(data!=BMA2x2_MODE_DEEP_SUSPEND)
		bma2x2_set_mode(BMA2x2_MODE_DEEP_SUSPEND);
}

void Bma250E_LowRate(void)
{
	//PRINTF("Bma250E_LowRate\r\n");
	uint8 data =0;
	DELAY_I2C;
	bma2x2_set_fifo_mode (0x00);
	bma2x2_get_fifo_mode (&data);
	if(data!=0x00)
		bma2x2_set_fifo_mode (0x00);	
	data=0;
	DELAY_I2C;
	bma2x2_set_int_slope (BMA2x2_INT1_SLOPE,0x01);
	bma2x2_get_int_slope (BMA2x2_INT1_SLOPE,&data);
	if(data!=0x01)
		bma2x2_set_int_slope (BMA2x2_INT1_SLOPE,0x01);

	data=0;
	DELAY_I2C;
	bma2x2_set_sleep_dur (BMA2x2_SLEEP_DUR_1S);//设置1s钟更新一次
	bma2x2_get_sleep_dur (&data);
	if(data!=BMA2x2_SLEEP_DUR_1S)
	bma2x2_set_sleep_dur (BMA2x2_SLEEP_DUR_1S);
	 
	bma2x2_set_Int_Enable(BMA2x2_Slope_X_Interrupt , 0x01 );
	bma2x2_set_Int_Enable(BMA2x2_Slope_Y_Interrupt , 0x01 );
	bma2x2_set_Int_Enable(BMA2x2_Slope_Z_Interrupt , 0x01 );
	DELAY_I2C;
	bma2x2_reset_interrupt (0x01);	
	
	data=0;
	DELAY_I2C;
	bma2x2_set_mode(BMA2x2_MODE_LOWPOWER1);
	bma2x2_get_mode (&data);
	if(data!=BMA2x2_MODE_LOWPOWER1)
		bma2x2_set_mode(BMA2x2_MODE_LOWPOWER1);
	Bma250EGotoLowPower =1;
}

void Bma250E_High_Rate(void)
{
	Bma250EGotoLowPower=0;
	Bma250E_Step_SetConfig();
}

void Bma250E_Sleep_SetConfig(void)
{
	uint8 data=0;
	DELAY_I2C;
	bma2x2_get_fifo_mode (&data);
...
void Bma250E_DoubleClick_SetConfig(void)
{
	uint8 data=0;
	//DOUBLE_CLICK
	data = BMA2x2_UNFILT_INT_SRC_TAP__MSK;			//unfilterd data for single/double tap interrupt 
	bma2x2_write_reg(BMA2x2_INT_SRC_REG,&data,0x01); 
	data = 8;        									//tap threshold 1 LSB for 250mg of 8-range
	bma2x2_write_reg(BMA2x2_TAP_THRES_REG,&data,0x01); 

	data = BMA2x2_TAP_SHOCK_DURN__MSK | 0x05 ;    	//tap timing 
	bma2x2_write_reg(BMA2x2_TAP_PARAM_REG,&data,0x01); 
	data = BMA2x2_EN_DOUBLE_TAP_INT__MSK ;        		//double    tap en 
	bma2x2_write_reg(BMA2x2_INT_ENABLE1_REG,&data,0x01); 
	data = BMA2x2_EN_INT1_PAD_DB_TAP__MSK ;     		//double  tap for int1 
	bma2x2_write_reg(BMA2x2_INT1_PAD_SEL_REG,&data,0x01); 

}
//BMA25xx Initialization, Register Configuration
void Bma250E_Step_SetConfig(void)
{
	uint8 data=0;
	data=0;
	...
	data=0;
	bma2x2_set_bandwidth(BMA2x2_BW_15_63HZ);
	bma2x2_get_bandwidth(&data);
	if(data!=BMA2x2_BW_15_63HZ)
		bma2x2_set_bandwidth(BMA2x2_BW_15_63HZ);

	data=0;
	DELAY_I2C;
	bma2x2_set_range(BMA2x2_RANGE_8G);
	bma2x2_get_range(&data);
	if(data!=BMA2x2_RANGE_8G)
		bma2x2_set_range(BMA2x2_RANGE_8G);
	
	bma2x2_set_shadow_dis(0x01);
	...
	data=0;
	DELAY_I2C;
	bma2x2_set_fifo_data_sel (0x00);
	bma2x2_get_fifo_data_sel (&data);
	if(data!=0x00)
		bma2x2_set_fifo_data_sel (0x00);

       data=0;
	DELAY_I2C;
	bma2x2_set_int_lvl (BMA2x2_INT1_LEVEL,0x00);
	bma2x2_get_int_lvl (BMA2x2_INT1_LEVEL,&data);
	if(data!=0x00)
		bma2x2_set_int_lvl (BMA2x2_INT1_LEVEL,0x00);
	
	data=0;
	DELAY_I2C;
	bma2x2_set_int_od (BMA2x2_INT1_OUTPUT,0x00);
	bma2x2_get_int_od (BMA2x2_INT1_OUTPUT,&data);
	if(data!=0x00)
	bma2x2_set_int_od (BMA2x2_INT1_OUTPUT,0x00);
	...
	data=0;
	DELAY_I2C;
	bma2x2_set_thr (BMA2x2_SLOPE_THRESHOLD,0x02);
	bma2x2_get_thr (BMA2x2_SLOPE_THRESHOLD,&data);
	if(data!=0x02)
	bma2x2_set_thr (BMA2x2_SLOPE_THRESHOLD,0x02);

	//Bma250E_DoubleClick_SetConfig();
	//nrf_gpio_cfg_sense_input(ACC_INT, NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_SENSE_LOW);
	data=0;
	DELAY_I2C;
	bma2x2_set_mode(BMA2x2_MODE_NORMAL);
	DELAY_I2C;
...
bool Bma250E_Init(void)
{
	gbma2x2.bus_write=write_bus_bma2xx;
	gbma2x2.bus_read=read_bus_bma2xx;
	gbma2x2.burst_read=read_burst_bma2xx;
	gbma2x2.delay_msec=delay_msec_bma2xx;	
	bma2x2_init(&gbma2x2);
	bma2x2_soft_reset();
	DelayMs(50);
	Bma250E_Step_SetConfig();
...
BMA2x2_RETURN_FUNCTION_TYPE Bma2x2GetFifoData(unsigned char *out_reg)
{
  int comres = C_BMA2x2_Zero_U8X;
  if (p_bma2x2==C_BMA2x2_Zero_U8X)
  {
  ...
  else
  {
    //GET FIFO DATA OUTPUT REGISTER
#if 0    
    comres = p_bma2x2->BMA2x2_BUS_READ_FUNC(p_bma2x2->dev_addr,BMA2x2_FIFO_DATA_OUTPUT_REG,out_reg,C_BMA2x2_Six_U8X*32);
#else
    comres = p_bma2x2->BMA2x2_BUS_READ_FUNC(p_bma2x2->dev_addr,BMA2x2_FIFO_DATA_OUTPUT_REG,out_reg,C_BMA2x2_Six_U8X);
#endif
  }
  return comres;
}

short GetAccData(unsigned char data[2])
{
  short temp=0;
  ...
  switch (V_BMA2x2RESOLUTION_U8R)
     {
     case 0:
        temp = BMA2x2_GET_BITSLICE(data[0],BMA2x2_ACC_Z12_LSB)| (BMA2x2_GET_BITSLICE(data[1],BMA2x2_ACC_Z_MSB)<<(BMA2x2_ACC_Z12_LSB__LEN));
        temp = temp << (sizeof(short)*8-(BMA2x2_ACC_Z12_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        temp = temp >> (sizeof(short)*8-(BMA2x2_ACC_Z12_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        break;
     case 1:
        temp = BMA2x2_GET_BITSLICE(data[0],BMA2x2_ACC_Z10_LSB)| (BMA2x2_GET_BITSLICE(data[1],BMA2x2_ACC_Z_MSB)<<(BMA2x2_ACC_Z10_LSB__LEN));
        temp = temp << (sizeof(short)*8-(BMA2x2_ACC_Z10_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        temp = temp >> (sizeof(short)*8-(BMA2x2_ACC_Z10_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        break;
     case 2:
        temp = BMA2x2_GET_BITSLICE(data[0],BMA2x2_ACC_Z8_LSB)| (BMA2x2_GET_BITSLICE(data[1],BMA2x2_ACC_Z_MSB)<<(BMA2x2_ACC_Z8_LSB__LEN));
        temp = temp << (sizeof(short)*8-(BMA2x2_ACC_Z8_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        temp = temp >> (sizeof(short)*8-(BMA2x2_ACC_Z8_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        break;
     case 3:
        temp = BMA2x2_GET_BITSLICE(data[0],BMA2x2_ACC_Z14_LSB)| (BMA2x2_GET_BITSLICE(data[1],BMA2x2_ACC_Z_MSB)<<(BMA2x2_ACC_Z14_LSB__LEN));
        temp = temp << (sizeof(short)*8-(BMA2x2_ACC_Z14_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        temp = temp >> (sizeof(short)*8-(BMA2x2_ACC_Z14_LSB__LEN + BMA2x2_ACC_Z_MSB__LEN));
        break;
     default:
        break;
