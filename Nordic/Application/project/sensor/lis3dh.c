...
#define STEP_MODE_WT_MARK_CNT 15		//16-1	计步状态
#define SLEEP_MODE_WT_MARK_CNT 1

//#define uart_Printf TRACE

u8_t LIS3DH_ReadReg_Multi(u8_t Reg, u8_t len, u8_t* Data)
{
	I2cMultipleByteRead(LIS3DH_I2C_ADDR<<1, Reg, Data, len);
	return 1;
}
...
u8_t LIS3DH_ReadReg(u8_t Reg, u8_t* Data) {
  ...
  I2cMultipleByteRead(LIS3DH_I2C_ADDR<<1, Reg, Data, 1);
  return 1;
}
...
u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data) {
  ...
  I2cMultipleByteWrite(LIS3DH_I2C_ADDR<<1, WriteAddr, &Data, 1);
  return 1;
}
...
void Lis_DelayMs(unsigned long ms)
{
        nrf_delay_ms(ms);
}

#define LIS_DELAY_I2C  Lis_DelayMs(2)
/*------------------------------------------
 lis3dh 唤醒正常计步
 flag = 0,睡眠   = 1,计步
--------------------------------------------*/
void lis3dh_wakeup(unsigned char flag)
{
	LIS_DELAY_I2C;
	LIS_DELAY_I2C;
	if(flag == 0)	//睡眠模式
	{
		...
		LIS3DH_SetMode(LIS3DH_LOW_POWER); //低功耗模式    
	}
	else
	{
		LIS3DH_SetODR(LIS3DH_ODR_50Hz);
		LIS3DH_SetMode(LIS3DH_NORMAL); //普通模式
	}
	LIS3DH_WriteReg(LIS3DH_CTRL_REG2, 0); // No HighPass filter
	LIS3DH_WriteReg(LIS3DH_CTRL_REG3, 0); // No interrupts
	LIS3DH_WriteReg(LIS3DH_CTRL_REG6, 0); // all defaults
	LIS3DH_FIFOModeEnable(LIS3DH_FIFO_STREAM_MODE);
	LIS3DH_SetFullScale(LIS3DH_FULLSCALE_8);        //8g
	LIS3DH_SetAxis(LIS3DH_X_ENABLE | LIS3DH_Y_ENABLE | LIS3DH_Z_ENABLE);
	//Set_CLICK(1);
	//LIS3DH_SetADCAux(MEMS_ENABLE);
}

unsigned char  lis3dh_checkDeviceId(void)
{
...
//进入Standby mode
void Lis3dh_Standby(void)
{
	lis3dh_wakeup(0);
}

void Lis3dh_Sleep_SetConfig(void)
{
	LIS3DH_FIFOModeEnable(LIS3DH_FIFO_BYPASS_MODE);	
}

void Lis3dh_Step_SetConfig(void)
{
	lis3dh_wakeup(1);
}
...
status_t Get_AccData(AxesRaw_t* buff)
{	
	i16_t value;
...
void Gsensor_receive_data(int16 * accbuf)
{
		AxesRaw_t  acc_data;		//read buffer
	
		for(uint8 acc_count=0; acc_count < STEP_BUFFER_FIFO_CNT; acc_count++)
		{
				Get_AccData(&acc_data);
				accbuf[acc_count*3+0] = acc_data.AXIS_Y;	//X
				accbuf[acc_count*3+1] = acc_data.AXIS_X;	//Y	 specific for Her by lcc
				accbuf[acc_count*3+2] = acc_data.AXIS_Z;
		}
}

void  Gsensor_read_accel(int16 * acc)
{
		AxesRaw_t acc_data;
		//uint8_t status = 0;
		//while((status&0x8)==0)
			//LIS3DH_GetStatusReg(&status);
		Get_AccData(&acc_data);
		*acc = acc_data.AXIS_Y;// X
		acc++;
		*acc = acc_data.AXIS_X;// Y specific for her by lcc & xf
		acc++;
		*acc = acc_data.AXIS_Z;
		acc++;
}

void Gsensor_get_fifo_framecount(unsigned char *framecount)
   {
		uint8_t tFifoCnt = 0,fifo_status=0;
		LIS3DH_ReadReg(LIS3DH_FIFO_SRC_REG,&fifo_status);		//FIFO buff count WTM OVRN EMPTY  FSS4 FSS3 FSS2 FSS1 FSS0
		tFifoCnt =((fifo_status&0x40)>>6) + (fifo_status&0x1f);				   //FIFO 里未读数据个数
		*framecount = tFifoCnt;
		//uart_Printf("tFifoCnt = %x\r\n",tFifoCnt);
   }



void Set_CLICK(uint8 On)
{
	LIS3DH_SetClickTHS(4);				/*各轴移动的触发门限 1~127*/
	LIS3DH_SetClickLIMIT(10);				/*时间门限*/
	LIS3DH_SetClickLATENCY(0);			/*时间延迟----中断持续时间*/
	LIS3DH_SetClickWINDOW(10);			/*时间窗口*/
	LIS3DH_SetClickCFG(0x20);
	LIS3DH_HPFClickEnable(MEMS_ENABLE);
	LIS3DH_SetInt1Pin(0x80);
	//nrf_gpio_cfg_sense_input(ACC_INT, NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_SENSE_HIGH);
}
// end of file ----------------------------------------------------
