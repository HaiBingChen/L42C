...
void mem_4_free(void *pv)
{
	...
	if( pv != NULL )
	{
		...
		pOS_ASSERT( ( pxLink->xBlockSize & xBlockAllocatedBit ) != 0 );
		pOS_ASSERT( pxLink->pxNextFreeBlock == NULL );
...
static void prvHeapInit( void )
{
	...
	pOS_ASSERT( ( ( ( unsigned long ) pxEnd ) & ( ( unsigned long ) portBYTE_ALIGNMENT_MASK ) ) == 0UL );
	pxEnd->xBlockSize = 0;
