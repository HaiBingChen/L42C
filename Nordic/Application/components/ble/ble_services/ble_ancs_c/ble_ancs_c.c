...
uint32_t ble_ancs_get_notif_attrs(const ble_ancs_c_t * p_ancs, uint8_t *pNotificationUID, uint8_t Type)
{
    tx_message_t * p_msg;
    uint32_t       index                    = 0;
    ...
    uint32_t p_uid = uint32_decode(pNotificationUID);
    index += uint32_encode(p_uid, &p_msg->req.write_req.gattc_value[index]);

    //Encode Attribute ID.
    if (m_ancs_attr_list[Type].get == true)
    {
        p_msg->req.write_req.gattc_value[index++] = Type;
        if(Type == BLE_ANCS_NOTIF_ATTR_ID_TITLE)
        {
            p_msg->req.write_req.gattc_value[index++] = 90;
            p_msg->req.write_req.gattc_value[index++] = 0;
        }
        else if(Type == BLE_ANCS_NOTIF_ATTR_ID_MESSAGE)
        {
                p_msg->req.write_req.gattc_value[index++] = 120;
                p_msg->req.write_req.gattc_value[index++] = 0;
        }
...
uint32_t ble_ancs_c_request_attrs(const ble_ancs_c_evt_notif_t * notif, uint8_t *pNotificationUID, uint8_t Type)
{
    uint32_t err_code;
    ...
    err_code      = ble_ancs_get_notif_attrs(mp_ble_ancs, pNotificationUID, Type);
