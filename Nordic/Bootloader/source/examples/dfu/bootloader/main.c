...
int main(void)
{
	...
	bool  should_enter_bootloader = (NRF_POWER->GPREGRET == 0x01) ? true: false;
	...
	UartInitial();
	dfu_start  = (app_reset|should_enter_bootloader);
	if (dfu_start || (!bootloader_app_is_valid(DFU_BANK_0_REGION_START)))
	{
		// Initiate an update of the firmware.
		nrf_delay_ms(600);
		SpiFlash_Init();
		OtaSendCmd(O_RESULT,3);
		err_code = bootloader_dfu_start();
		if(m_update_status==BOOTLOADER_COMPLETE){
			/*升级成功*/
		}else if(m_update_status==BOOTLOADER_TIMEOUT){
			/*升级失败*/
		}
		StoreOtaResult();
		nrf_delay_ms(1200);
