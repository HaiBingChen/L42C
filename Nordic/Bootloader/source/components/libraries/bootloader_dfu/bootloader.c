...
bootloader_status_t      m_update_status;        /**< Current update status for the bootloader module to ensure correct behaviour when updating settings and when update completes. */
uint8_t AppscommRetryCount =1;
...
static void wait_for_events(void)
{
	for (;;)
	{
		...
		if(m_update_status == BOOTLOADER_ERASE_NORDIC_OK)			/*擦除内部flash完毕*/
		{									
			Appscomm_Stop_App();			
		}
		else if(m_update_status == BOOTLOADER_STOP_APP_OK )		/*不让从APP启动，等写入并校验完毕*/
		{
			Appscomm_Write_Stop_App();
		}
		else if(m_update_status==BOOTLOADER_STOP_APP_WRITE_OK ||m_update_status == BOOTLOADER_WRITE_NORDIC_NEXT)	/*写内部flash*/
		{
			Appscomm_Write_Nordic();
		}
		else if(m_update_status==BOOTLOADER_WRITE_NORDIC_OK)		/*写内部flash完毕校验*/
		{
			if(Appscomm_app_is_valid(DFU_BANK_0_REGION_START))	/*校验成功*/
			{
				Appscomm_Erase_Settings();
			}
			else if(AppscommRetryCount>0)							/*校验不成功，再尝试一次*/
			{
				AppscommRetryCount--;
		...
		else if(m_update_status==BOOTLOADER_ERASE_SETTING_OK)		/*校验成功,允许从APP启动*/
		{
			Appscomm_Write_Settings();
		}
		else if(m_update_status==BOOTLOADER_WRITE_SETTING_OK)		/*从APP启动*/
		{
			OtaResult &= (~(UPDATE_NORDIC));
			return;
		}	 
	}
}
...
void Appscomm_Erase_Settings(void)
{
	...
	APP_ERROR_CHECK(err_code);
	m_update_status = BOOTLOADER_ERASE_SETTING;
}

 void Appscomm_Write_Settings(void)
{
	uint32_t err_code;
	static bootloader_settings_t  AppscommSettings;
	AppscommSettings.bank_0_crc  = 0;
	AppscommSettings.bank_0_size = CustomNordic_Size;
	AppscommSettings.bank_0      = BANK_VALID_APP;
	AppscommSettings.bank_1      = BANK_INVALID_APP;	
	err_code = pstorage_store(&m_bootsettings_handle,
							(uint8_t *)(&AppscommSettings),
							sizeof(bootloader_settings_t),
							0);
	APP_ERROR_CHECK(err_code);	
	m_update_status = BOOTLOADER_WRITE_SETTING;
}
...
bool Appscomm_app_is_valid(uint32_t app_addr)
{
	...
	if ((CustomNordic_Crc!=0)&&(CustomNordic_Size!=0))
	{
		image_crc = crc16_compute((uint8_t *)DFU_BANK_0_REGION_START,
									CustomNordic_Size,
									NULL);
	}
	success = (image_crc == CustomNordic_Crc);
	return success;
}
...
void Appscomm_Stop_App(void)
{
	uint32_t err_code = pstorage_clear(&m_bootsettings_handle, sizeof(bootloader_settings_t));
	APP_ERROR_CHECK(err_code);
	m_update_status = BOOTLOADER_STOP_APP;
}
 ...
 void Appscomm_Write_Stop_App(void)
{
	...
	static bootloader_settings_t  AppscommSettings;
	AppscommSettings.bank_0_crc  = 0;
	AppscommSettings.bank_0_size = 0;
	AppscommSettings.bank_0      = BANK_INVALID_APP;
	AppscommSettings.bank_1      = BANK_INVALID_APP;	
	err_code = pstorage_store(&m_bootsettings_handle,
							(uint8_t *)(&AppscommSettings),
							sizeof(bootloader_settings_t),
							0);
	APP_ERROR_CHECK(err_code);	
	m_update_status = BOOTLOADER_STOP_APP_WRITE;
}
