...
void Appscomm_Erase_Nordic(void)
{
	uint32_t err_code;
	WriteSize =0;
	dfu_timer_restart();
	err_code    = pstorage_clear(&m_storage_handle_app, CustomNordic_Size);
	APP_ERROR_CHECK(err_code); 
	m_update_status = BOOTLOADER_ERASE_NORDIC;
}

void Appscomm_Write_Nordic(void)
{
	uint32_t err_code;
	dfu_timer_restart();
	if((WriteSize+512)>=CustomNordic_Size)
	{
		uint32_t LastWriteSize=CustomNordic_Size-WriteSize;
		uint8_t LastWriteSizeRes =0;
		LastWriteSizeRes = LastWriteSize%4;
		if(LastWriteSizeRes!=0)
		{
			LastWriteSize +=(4-LastWriteSizeRes);
		}
		SpiFlash_Read((CustomNordic_Addr+WriteSize),NordicBuf,LastWriteSize);
		err_code = pstorage_store(&m_storage_handle_app,NordicBuf,LastWriteSize,WriteSize);
		APP_ERROR_CHECK(err_code); 			
		m_update_status = BOOTLOADER_WRITE_NORDIC_LAST;
		return;
	}
	SpiFlash_Read((CustomNordic_Addr+WriteSize),NordicBuf,512);
	err_code = pstorage_store(&m_storage_handle_app,NordicBuf,512,WriteSize);
	APP_ERROR_CHECK(err_code); 
	WriteSize+=512;
	m_update_status = BOOTLOADER_WRITE_NORDIC;
}
