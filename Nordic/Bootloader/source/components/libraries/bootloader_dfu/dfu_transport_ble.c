...
uint8_t DEVICE_NAME[9]="42#00000";   
//#define DEVICE_NAME                          "L50AOta"                                               /**< Name of device. Will be included in the advertising data. */
...
static void on_dfu_pkt_write(ble_dfu_t * p_dfu, ble_dfu_evt_t * p_evt)
{
    ...
    switch (m_pkt_type)
    {
	 ...
	 case PKT_TYPE_USER:
	     {
		  uint32_t           err_code;	
	         err_code = ble_dfu_response_send(p_dfu, BLE_DFU_USER_PROCEDURE, BLE_DFU_RESP_VAL_SUCCESS);	
	         APP_ERROR_CHECK(err_code);
	     }
	     break;	
